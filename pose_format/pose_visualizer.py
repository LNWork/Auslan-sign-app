...
class PoseVisualizer:
    ...
    *cv2 : module
    ...
    def _draw_frame(self, frame: ma.MaskedArray, frame_confidence: np.ndarray, img,
                    transparency: bool = False, title: str = "") -> np.ndarray:
        """
        Draw frame of pose data of an image and add title at the bottom.
        ...
        title : str, optional
            The title to be displayed at the bottom of the frame.

        Returns
        -------
        np.ndarray
            Image with drawn pose data and title text at the bottom.
        """
        
        # Existing code to draw the pose...
    ...
    def draw_on_video(self, background_video, max_frames: int = None, blur=False, title: str = ""):
        """
        Draw pose on a background video and add a title at the bottom of each frame.

        Parameters
        ----------
        background_video : str or iterable
            Path to the video file or iterable of video frames.
        max_frames : int, optional
            Maximum number of frames to process. If None, all frames will be processed.
        blur : bool, optional
            If True, applies a blur effect to the video.
        title : str, optional
            The title to display at the bottom of each frame.

        Yields
        ------
        np.ndarray
            Frames with overlaid pose data and title.
        """
        ...
        for frame, confidence, background in itertools.islice(
            ...
            background = self.cv2.resize(background, (self.pose.header.dimensions.width, self.pose.header.dimensions.height))

            if blur:
                background = self.cv2.blur(background, (20, 20))

            # Pass the title to _draw_frame
            yield self._draw_frame(frame, confidence, background, title=title)
